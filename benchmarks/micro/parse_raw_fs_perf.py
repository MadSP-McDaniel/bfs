...
plt.style.use("bmh")
p = os.getenv("BFS_HOME") + "/benchmarks/new_bench/o/"
# sub = False  # separate plots or single
# fig, axes = plt.subplots(nrows=2, ncols=2)
bfs_res = pd.read_csv(p + "raw_bfs.csv", squeeze=True).set_index("config")
# bfs_res.insert(0, "fs_type", "bfs_seq")
gramine_res = pd.read_csv(p + "raw_gramine.csv", squeeze=True).set_index("config")
