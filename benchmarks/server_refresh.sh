...
if [ "${fs_type}" = "bfs" ]; then
    ...
    mkdir -p $BFS_HOME/benchmarks/$bench_type/output &>/dev/null # for server logs
    $BFS_HOME/build/bin/bfs_server &>$BFS_HOME/benchmarks/$bench_type/output/${fs_type}_server_current.log &
    sleep 10
    ...
    # rm -rf /tmp/filebench-shm-* &>/dev/null # clear old filebench caches (edit: dont need on server)
    mkdir -p $BFS_HOME/benchmarks/$bench_type/output &>/dev/null # for server logs
    $BFS_HOME/build/bin/bfs_server_ne &>$BFS_HOME/benchmarks/$bench_type/output/${fs_type}_server_current.log &
    sleep 10
